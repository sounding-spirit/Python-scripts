...
df=df.rename(columns={"ssdl:pid": "PID", "ssdl:placeOfPublication": "location", "ssdl:dateOfPublication": "Date"})
df["location"]=df["location"].astype(str)
df["Date"]=df["Date"].astype(str)
df["ssdl:spatialCoverageGettyUri"]=df["ssdl:spatialCoverageGettyUri"].astype(str)
...
for x in df.index:
    ...
    df.loc[x, "ssdl:spatialCoverageGettyUri"]=df.loc[x, "ssdl:spatialCoverageGettyUri"].replace("\n",";")
    for uri in df.loc[x, "ssdl:spatialCoverageGettyUri"].split(";"):
        coor = get_coor(uri)
...
df['Long'] = long
df['Lati'] = lati
df = df.drop('ssdl:spatialCoverageGettyUri', axis=1)
